...
<Note>
  Mainnet currently supports 16 chains across domains 0-22, while testnet supports 16 chains. The function internally calls either `GetMainnetChains()` or `GetTestnetChains()` based on the `testnet` parameter.
</Note>
